...
function HomePage() {
    ...
    return (
        <Section style={{padding:"0"}}>
            <Image src={banner} />
            <Heading size={1} mt={6} textAlign={'center'}>Favorite Products</Heading>
            {breakfasts.length > 0 ? (
                <Columns m={4} multiline gap={8}>
                    {breakfasts && breakfasts.sort((a,b)=>descendingComparator(a,b,"likes")).slice(0,8).map((breakfast) => {
                        return (
                ...
                <>
                    <Container backgroundColor='light' mt={6} style={{ width: "35vw", margin: "auto", padding: "20px 20px", borderRadius: "10px" }}>
                        <Notification>
                            ...
                            {userContext.details ?
                                (<Heading subtitle>Click < a href="/admin/add-breakfast">here</a> to create a Breakfast</Heading>) :
